...
class UncertaintyEstimator:
    ...
    def gather_scores(self, ids_sampled=None):
        ...
        if ids_sampled is not None:
            df = {id1: {id2: df[id1][id2] for id2 in df[id1] if id2 in ids_sampled} for id1 in ids_sampled if id1 in df}
    ...
    def _run_bootstrap_dbn(self):
        ids_sampled = set([id for id in self.scores.keys() if np.random.choice([0,1]) == 1])
        
        scores, labels = self.gather_scores(ids_sampled)
        fprb, tprb, thresholds = metrics.roc_curve(y_true = labels, y_score = scores, drop_intermediate=False)
    ...
    def compute_binerror_metrics(self, threshold):
        ...
        tn, fp, fn, tp = metrics.confusion_matrix(y_true = labels, y_pred = np.array(scores) > threshold).ravel()
    ...
    def _estimate_plugin(self, threshold):
        ...
        fnr_by_id = [(np.mean(self.errors[id][id]) - fnr) * len(self.errors[id][id])
                    for id in self.errors.keys() if id in self.errors[id] and len(self.errors[id][id]) > 0]
        m_by_id = [len(self.errors[id][id]) for id in self.errors.keys() if id in self.errors[id]]
        var_fnr = np.sum(np.square(fnr_by_id)) / np.square(np.sum(m_by_id))

        # estimate fpr variance
        m_by_id = [[len(self.errors[id1][id2]) for id2 in self.errors[id1] if id2 != id1]
                for id1 in self.errors.keys()]
        fpr_by_id = [[(np.mean(self.errors[id1][id2]) - fpr) * len(self.errors[id1][id2])
                    for id2 in self.errors[id1] if id2 != id1] for id1 in self.errors.keys()]
    ...
    def compute_variance(self, threshold, estimator="plugin"):
        ...
        if estimator == "plugin":
            ...
            if self.boot_results is None: ValueError("Bootstrap needs to be run first")
            indices_threshold = [np.argmin(np.abs(boot_th[2] - threshold)) for boot_th in self.boot_results]
            var_fpr = np.var([self.boot_results[i][0][indices_threshold[i]] for i in range(len(indices_threshold))])
            var_fnr = np.var([self.boot_results[i][1][indices_threshold[i]] for i in range(len(indices_threshold))])
    ...
    def get_binerror_ci(self, threshold=None, var_fnr=None, var_fpr=None, alpha=0.05):
        ...
        if var_fnr is None or var_fpr is None:
            ...
            var_fnr, var_fpr = self.compute_variance(threshold = threshold, estimator = "plugin")
        
        fpr_interval = self._get_ci_wilson(n_errors = self.fp, n_instances = self.fp + self.tn, mean_variance = var_fpr, alpha = alpha)
        fnr_interval = self._get_ci_wilson(n_errors = self.fn, n_instances = self.fn + self.tp, mean_variance = var_fnr, alpha = alpha)
    ...
    def get_roc_ci(self, target_fpr=None, alpha=0.05):
        ...
        fpr, tpr, thresholds = metrics.roc_curve(y_true=labels, y_score=scores, drop_intermediate=False if target_fpr is not None else True)
        ...
        confint_auc = np.percentile(auc_boot, q=[alpha/2 * 100, 100 - (alpha/2 * 100)], axis=0)
        
        if target_fpr is not None:
            tpr_at_fpr_boot = [np.interp(target_fpr, fprb, tprb).tolist() for fprb, tprb, _ in self.boot_results]
        else:
            tpr_at_fpr_boot = [np.interp(fpr, fprb, tprb).tolist() for fprb, tprb, _ in self.boot_results]
        confint = np.percentile(tpr_at_fpr_boot,  q=[alpha/2 * 100, 100 - (alpha/2 * 100)], axis=0)
