...
class TestUncertaintyEstimator(unittest.TestCase):
    ...
    def test_uncertainty_estimator(self):
        ...
        auc = uq.compute_auc()
        ci_bin_error_plugin = uq.get_binerror_ci(
            threshold, var_fnr, var_fpr, alpha=0.05
        )
        ci_bin_error_boot = uq.get_binerror_ci(
            threshold, var_fnr_boot, var_fpr_boot, alpha=0.05
        )
